...
\section ..
   ...
   \code 
            ...
            libname riskdata postgres server="sas-crunchy-data-cdspostgres" port=5432
            user=dbmsowner password='<pass>' database=SharedServices
            schema="public";


            Lua submission :
            local ds = require(sas.risk.irm.lua.core_data_service)
            //Default parition strategy is ROWS. Per partition 1 million rows be default.
            ds.generatePartitions("work","cars","riskdata","carsNewRows",
            ...
            ds.generatePartitions("work","cars","riskdata","carsNewByVars",
                                    { partitionByVars = "type origin" ,
                                        primaryKey = "obs type origin" ,
                                        strategyType = "BYVARS" ,
                                    } ,true)
            ...
            ds.generatePartitions("work","cars","riskdata","carsNewFixed",
                                    { partitionByVars = "type origin" ,
                                        primaryKey = "obs type origin" ,
                                        strategyType = "FIXED" ,
                                        partitionCount= 5
            ...
            ds.generatePartitions("work","cars","riskdata","carsNewNone",
                                    { partitionByVars = "type origin" ,
                                        primaryKey = "obs type origin" ,
                                        strategyType = "NONE" ,
                                    } ,true)
...
-- @param Takes any number of strings to print sas.print.
---@return nil
...
---@param fullDSName string  <libName.Viewname>
---@return number
M.sasDeleteView= function(fullViewName,sqlOptions)
    ...
    rc=sas.submit(code,{fullViewName=fullViewName,sqlOptions=sqlOptions},nil,2000)
...
---@param fullDSName string  <libName.DataSetName>
---@param partitionByVars string : space seperrated list of partition vars.
---@return number
M.partitionByVars=function(fullDSName,partitionByVars)
    ...
    partitionByVars=sas.trim(partitionByVars)
    partitionByVars=string.gsub(partitionByVars," +","||")
    local parts_cnt=nil
    local code = [[
    proc sql noprint;
     select count(distinct @partitionByVars@ ) into : parts_cnt from @fullDSName@;
    quit;
   ]]
...
---                         }
M.generatePostgresSQLPartition=function(sourceLibname,sourceDSName,targetLibname,targetDSName,parameters)
    ...
    if parameters.partitionCount > 0 and (parameters.partitionByVars ~= nil and parameters.partitionByVars ~= '') then
        ...
        parameters.partitionByVars=sas.trim(parameters.partitionByVars)
        ...
        for i=0,parameters.partitionCount-1,1 do
            partitionStr =partitionStr.."create table " ..fullTargetDS.. "_"..i.." partition of "..fullTargetDS.." for values  with (modulus "..parameters.partitionCount.." , remainder "..i..");\n"
        end
    ...
    if parameters.primaryKey ~= nil and parameters.primaryKey ~= '' then
        --Trimming before we use gsub
        parameters.primaryKey=sas.trim(parameters.primaryKey)
        local _tmp_=string.gsub(parameters.primaryKey," +",",")
        primaryKeyStr="Alter table "..fullTargetDS.." add primary key (".._tmp_..");"
    ...
    if parameters.indexList ~= nil and parameters.indexList ~= '' then
        --Trimming before we use gsub
        parameters.indexList=sas.trim(parameters.indexList)
        for i,v in ipairs(string.split(parameters.indexList,",")) do
            ...
            local _tmp_="create index on "..fullTargetDS.." ("..string.gsub(M.trimAll(v)," +",",")..");"
...
M.generatePartitions=function(sourceLibname,sourceDSName,targetLibname,targetDSName,parameters,replace,dbType)
    local rc=nil
    local status=nil
    ...
    if dbType == defaultDBType then
        rc = M.generatePostgresSQLPartition(sourceLibname,sourceDSName,targetLibname,targetDSName,parameters)
    else
        rc=nil
    end
