...
%macro core_rest_get_token(url =
   ...
   %if %sysevalf(%superq(logonHost) ne, boolean) %then
   ...
   %let fref = %core_get_unique_ref(prefix = tmp, engine = temp, debug = &debug.);
   ...
   %if(&sys_prochttp_status_code. ne 200) %then %do;
      ...
      %core_print_file(file = %sysfunc(pathname(&fref.))
   ...
   %let libref = %core_get_unique_ref(type = lib, engine = JSON, args = fileref = &fref.);
