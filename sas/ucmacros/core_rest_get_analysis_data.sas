...
\anchor core_rest_get_analysis_data
   ...
   \details
   This macro sends a GET request to <b><i>\<host\>:\<port\>/riskCirrusObjects/objects/&solution./analysisData</i></b> and collects the results in the output table. \n
   ...
   \code
      %let accessToken=;
      %core_rest_get_analysis_data(host = <host>
                                      , port = <port>
                                      , username = <userid>
                                      , password = <pwd>
                                      , outds = analysis_data
