...
%core_append_many(file=base,append=appends,overwrite=Y);
filename base clear;
filename f1 clear;
...
%core_append_many(file=base,append=appends);
...
%macro core_append_many(file =, append =, overwrite = N, newline = %str('0D0A'x));
